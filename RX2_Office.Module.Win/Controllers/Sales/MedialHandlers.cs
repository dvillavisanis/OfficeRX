    ...
    public class MediaHandlers : INotifyPropertyChanged, IDisposable
    {
        ...
        private static Stream LoadRingbackStream()
        {
            Stream ringback = Assembly.GetExecutingAssembly().GetManifestResourceStream(
                "CallCenter.Module.Win.Images.ringback.wav");

            if (ringback == null)
                throw new Exception("Cannot load default ringback.wav.");
        ...
        private Stream LoadRingtoneStream()
        {
            ...
            Stream basicRing = Assembly.GetExecutingAssembly().GetManifestResourceStream(
                "CallCenter.Module.Win.Images.basic_ring.wav");
        ...
        /// </summary>
        /// <returns></returns>
        public List<OzResolution> GetVideoResolutions()
        {
            var resolutions = new List<OzResolution>();
            ...
            resolutions.AddRange(capabilities.Select(cap => new OzResolution(cap.Resolution.Width, cap.Resolution.Height)));
